...
export default function DeckPurgeDialog({
  deckHash,
  deckName,
  onCancel,
  onConfirm,
  ...
  useEffect(() => {
    ...
    getDeckContents(deckHash)
      ...
      .catch((err) => {
        if (!cancelled) setLoadError({ message: err.message || null });
      });
    return () => {
      cancelled = true;
    };
  ...
  return (
    ...
    >
      <div className="deck-purge">
        {loadError && (
          <p className="deck-purge__error">
            {loadError.message ?? t("Could not read the deck.")}
          </p>
        )}
        {error && <p className="deck-purge__error">{error}</p>}
        {!summary && !loadError && (
          <p className="deck-purge__muted">
            {t("Checking what’s in this deck…")}
          </p>
        )}
        ...
        {summary && (
          <>
            {summary.total === 0 && (
              <p>
                {t("This deck is empty. Only the deck itself will be removed.")}
              </p>
            )}
            ...
            {summary.total > 0 && doomed === 0 && (
              <p className="deck-purge__lead">
                {t(
                  "No cards will be deleted — every card here is also in another deck. Only the deck itself will be removed.",
                )}
              </p>
            )}
            ...
            {doomed > 0 && (
              <>
                <p className="deck-purge__lead">
                  {tp(
                    "{n} card will be permanently deleted:",
                    "{n} cards will be permanently deleted:",
                    doomed,
                  )}
                </p>
                <ul className="deck-purge__breakdown">
                  {doomedStandalone > 0 && (
                    <li>{t("{n} standalone", { n: doomedStandalone })}</li>
                  )}
                  {doomedAnchored > 0 && (
                    <li>
                      {tp(
                      ...
                      —{" "}
                      <span className="deck-purge__docs">
            ...
            {summary.shared > 0 && (
              <label className="deck-purge__shared">
                ...
                <span>
                  {tp(
                    "Also delete {n} card that is also in",
