  ...
  switch (problem.code) {
    case "required":
      return problem.field === "name"
        ...
        : t("An email is required.");
    case "invalid-chars":
      return t("Contains characters that cannot be used here.");
    case "too-long":
      return t("Too long (max 128 characters).");
    case "not-an-address":
...
function sourceText(source, t) {
  switch (source) {
    case "vault":
      return t("from this vault’s override");
    case "global":
      return t("from your identity");
    default:
      return t(
        "from your computer account — set a name and email to change it",
      );
  }
}
...
export default function IdentitySection({ connection }) {
  ...
  const overrideProblem = override
    ? problemText(identityError(override), t)
    : null;
  ...
  return (
    <section className="config-section">
      ...
      <p className="config-hint">
        {t(
          "The name and email stamped on documents you create and on every entry in the vault history.",
        )}
      </p>

      {effective && (
        <p className="identity-current">
          {t("Stamping new work as")} <code>{effective.author}</code>{" "}
          <span className="identity-current__source">
            {sourceText(effective.source, t)}
          </span>
        </p>
      )}

      {error && (
        <p className="identity-msg identity-msg--error" role="alert">
          {error}
        </p>
      )}
      ...
      <table className="config-table">
        <tbody>
          <tr>
            ...
            <td>
              <input
                ...
                autoComplete="off"
                placeholder={
                  effective?.source === "default" ? effective.name : undefined
                }
                onChange={(e) =>
                  setGlobal((g) => ({ ...g, name: e.target.value }))
                }
              />
            </td>
          </tr>
          <tr>
            <td>
              <label htmlFor="identity-email">{t("Email")}</label>
            </td>
            <td>
              <input
                id="identity-email"
                ...
                autoComplete="off"
                placeholder={
                  effective?.source === "default" ? effective.email : undefined
                }
                onChange={(e) =>
                  setGlobal((g) => ({ ...g, email: e.target.value }))
                }
              />
            </td>
          </tr>
      ...
      {globalProblem && (
        <p className="identity-msg identity-msg--error">{globalProblem}</p>
      )}
      ...
      <div className="identity-actions">
        <button
          ...
          onClick={() =>
            run("save", () => setIdentity(global), t("Identity saved."))
          }
        >
      ...
      <p className="config-hint identity-toggle__hint">
        {t(
          "Keeps a separate name and email for this vault only — a work address on a work vault, say.",
        )}
      </p>
        ...
        <>
          <table className="config-table">
            <tbody>
              <tr>
                <td>
                  <label htmlFor="identity-vault-name">{t("Name")}</label>
                </td>
                <td>
                  <input
                    id="identity-vault-name"
                    value={override.name}
                    autoComplete="off"
                    onChange={(e) =>
                      setOverride((o) => ({ ...o, name: e.target.value }))
                    }
                  />
                </td>
              </tr>
              <tr>
                <td>
                  <label htmlFor="identity-vault-email">{t("Email")}</label>
                </td>
                <td>
                  <input
                    id="identity-vault-email"
                    type="email"
                    value={override.email}
                    autoComplete="off"
                    onChange={(e) =>
                      setOverride((o) => ({ ...o, email: e.target.value }))
                    }
                  />
                </td>
              </tr>
          ...
          {overrideProblem && (
            <p className="identity-msg identity-msg--error">
              {overrideProblem}
            </p>
          )}
          ...
          <div className="identity-actions">
            <button
              ...
              onClick={() =>
                run(
                  "save-override",
                  () => setVaultIdentity(vaultId, override),
