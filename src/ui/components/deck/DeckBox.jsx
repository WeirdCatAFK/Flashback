 ...
 * default deck is kraft. `size="lg"` is the deck page's; the grid uses the default.
 *
...
export default function DeckBox({ color = 'kraft', count = 0, size = 'md', className = '' }) {
  const tops = Math.min(4, count);
  const sleeve = color === 'kraft' ? 'var(--color-kraft)' : `var(--color-box-${color})`;
  return (
    <div className={`deck-box deck-box--${size}${className ? ` ${className}` : ''}`} style={{ '--sleeve': sleeve }} aria-hidden="true">
