  ...
  return {
    ...
    reversible: [
      { key: "front", label: t("Term"), hint: t("Asked in either direction") },
      {
        key: "back",
        label: t("Definition"),
        hint: t("Asked in either direction"),
      },
    ],
    cloze: [
    ...
    type_answer: [
      { key: "front", label: t("Question"), hint: t("Shown to the user") },
      {
        ...
        hint: t("Typed and checked"),
      },
      {
        key: "back",
        label: t("Notes"),
        hint: t("Shown after checking — never compared"),
      },
    ],
    custom: [],
  };
  ...
  return [
    {
      key: "front_img",
      label: t("Front image"),
      hint: t("with the question"),
      opposite: "back_img",
    },
    {
      key: "front_sound",
      label: t("Front sound"),
      hint: t("plays with the question"),
      opposite: "back_sound",
    },
    {
      key: "back_img",
      label: t("Back image"),
      hint: t("with the answer"),
      opposite: "front_img",
    },
    {
      key: "back_sound",
      label: t("Back sound"),
      hint: t("plays with the answer"),
      opposite: "front_sound",
    },
  ];
}

/** type_answer has no media in FlashcardForm, and custom stores raw HTML. */
const typeHasMedia = (cardType) =>
  cardType === "basic" || cardType === "reversible" || cardType === "cloze";
...
export default function AnkiMappingModal({
  report,
  filename,
  onCancel,
  onConfirm,
  ...
  const moveField = (fieldName, toSlot) => {
    setMappings((prev) => {
      ...
      for (const slot of ALL_SLOTS) {
        slots[slot] = (current.slots[slot] ?? []).filter(
          (n) => n !== fieldName,
        );
      }
  ...
  const chip = (fieldName, slot, opposite = null) => (
    ...
    >
      ...
      <span className="anki-map__chip-sample">
        {plainSample(valueOf(fieldName)).slice(0, 40) || "—"}
      </span>
      {opposite && (
        <button
          type="button"
  ...
  const previewFields = {
    front: joined("front"),
    back: joined("back"),
    clozeText: joined("front").replace(
      /\{\{c\d+::([^:}]+)(?:::[^}]*)?\}\}/g,
      "{{$1}}",
    ),
  ...
  const rawOf = (slot) =>
    (mapping.slots[slot] ?? []).map((n) => valueOf(n)).join("\n");
  ...
  for (const slot of ["front_img", "front_sound", "back_img", "back_sound"]) {
    ...
    previewMedia[slot] = ref
      ? ankiSessionMediaSrc(report.sessionId, ref)
      : null;
  }

  const previewCard = previewCardFor(
    mapping.cardType,
    previewFields,
    previewMedia,
  );
  ...
  const totalCards = notetypes.reduce(
    (sum, nt) => sum + (nt.noteCount ?? 0),
    0,
  );
        ...
        <div className="anki-map__footer">
          ...
          <div className="anki-map__footer-actions">
            ...
            >
              {importing
                ? t("Importing…")
                : tp("Import {n} card", "Import {n} cards", totalCards)}
            </button>
          </div>
        </div>
      }
          ...
          <div className="anki-map__target">
            ...
            {mapping.cardType === "custom" ? (
              <p className="anki-map__note anki-map__note--block">
                {t(
                  "Custom cards keep the note’s original Anki HTML exactly as it renders, so there is nothing to map. Media is still imported.",
                )}
              </p>
            ) : (
              <div className="anki-map__slots">
                {SLOTS_BY_TYPE[mapping.cardType].map((slot) => (
                  <div
                    key={slot.key}
                    className="anki-map__slot"
                    {...dropHandlers(slot.key)}
                  >
                    ...
                    <div className="anki-map__slot-body">
                      {(mapping.slots[slot.key] ?? []).length === 0 && (
                        <span className="anki-map__dropzone">
                          {t("Drop a field here")}
                        </span>
                      )}
                      {(mapping.slots[slot.key] ?? []).map((n) =>
                        chip(n, slot.key),
                      )}
                    </div>
                  </div>
                ...
                {typeHasMedia(mapping.cardType) && (
                  <div className="anki-map__media">
                    {MEDIA_SLOTS.filter(
                      (m) =>
                        mapping.cardType !== "cloze" ||
                        m.key.startsWith("front"),
                    ).map((m) => (
                      <div
                        key={m.key}
                        className="anki-map__slot anki-map__slot--media"
                        {...dropHandlers(m.key)}
                      >
                        <div className="anki-map__slot-head">
                          <span className="anki-map__slot-label">
                            {m.label}
                          </span>
                        ...
                        <div className="anki-map__slot-body">
                          ...
                          {(mapping.slots[m.key] ?? []).map((n) =>
                            chip(
                              n,
                              m.key,
                              mapping.cardType === "cloze" ? null : m.opposite,
                            ),
                          )}
                        </div>
                      </div>
