/**
 * Server — the tab a remote Flashback Server gets: the handshake facts, your
 * own account, and for an admin the people table (roles, tokens, seats, and a
 * way into each person's progress). Data comes from useAccounts.js.
 */
...
import { roleLabels, roleBlurbs, fmtDate, grantable } from './roles.js';
    ...
    <div className="srv-token" role="alert">
      ...
      <div className="srv-token__actions">
        <button type="button" className="btn btn--primary"
          onClick={async () => {
            try { await navigator.clipboard.writeText(token); setCopied(true); } catch { }
          }}>
          {copied ? t("Copied") : t("Copy")}
        </button>
        <button type="button" className="btn" onClick={onDismiss}>{t("Done")}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <tr className={account.active ? "" : "srv-row--inactive"}>
      ...
      <td className="srv-actions">
        ...
        {live.length > 0 && (
          <button type="button" className="btn btn--sm btn--danger-quiet" disabled={!!busy}
...
function People({ onViewProgress }) {
  ...
  const { accounts, you, limit, loading, error, busy, run, activeCount, full } = useAccounts();
  const [reveal, setReveal] = useState(null);
  const [form, setForm] = useState({ name: "", email: "", role: ROLES.READER });
  const [showForm, setShowForm] = useState(false);

  const canGrant = useCallback((target) => grantable(role, target), [role]);
  ...
  return (
    <section className="srv-section">
      <div className="srv-section__head">
        ...
        <button type="button" className="btn" disabled={full && !showForm}
      ...
      {showForm && (
        <div className="srv-form">
          ...
          <button type="button" className="btn btn--primary" disabled={busy === "create"}
      ...
      {can("rotatePureToken") && (
        <div className="srv-danger">
          ...
          <button type="button" className="btn btn--danger" disabled={busy === "pure"}
            onClick={async () => {
              if (!(await confirm({
