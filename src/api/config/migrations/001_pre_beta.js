...
export async function up(db) {
    ...
    const deckCols = (await db.prepare("PRAGMA table_info('Decks')").all()).map(c => c.name);
    if (!deckCols.includes('node_id')) {
        await db.prepare("ALTER TABLE Decks ADD COLUMN node_id INTEGER REFERENCES Nodes(id)").run();
    }
    ...
    const deckConnTypeId = (await db.prepare("SELECT id FROM ConnectionTypes WHERE name = 'deck'").get())?.id;
    ...
    if (deckNodeTypeId) {
        ...
        for (const deck of await db.prepare("SELECT id FROM Decks WHERE node_id IS NULL").all()) {
            const { lastInsertRowid } = await insertNode.run(deckNodeTypeId);
            await updateDeck.run(lastInsertRowid, deck.id);
        }
    }
    ...
    if (deckConnTypeId) {
        ...
        for (const e of entries) {
            await insertConn.run(e.deck_node_id, e.card_node_id, deckConnTypeId);
        }
    }
}
