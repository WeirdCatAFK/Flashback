...
const clipName = (name) =>
  name.length > OPTION_NAME_MAX ? `${name.slice(0, OPTION_NAME_MAX - 1).trimEnd()}…` : name;
 ...
 */
export default function ProgressScopePicker({ value, onChange, className, note = false }) {
  const { t } = useT();
  ...
  useEffect(() => {
    ...
    listAccounts()
      .then((data) => {
        ...
        setAccounts((data.accounts ?? []).filter((a) => a.active && a.id !== me));
      })
  ...
  // A person deactivated after being chosen drops out of the list; treat that as "you".
  const current = value && accounts.some((a) => a.id === value.id) ? value.id : "";
    ...
    <div className={className ? `scope-picker ${className}` : "scope-picker"}>
      <label className="scope-picker__control">
        ...
        <select
          ...
          onChange={(e) => {
            const picked = accounts.find((a) => a.id === e.target.value) ?? null;
            onChange(picked ? { id: picked.id, name: picked.name, role: picked.role } : null);
          }}
        >
