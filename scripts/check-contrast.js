...
function deltaE(a, b, kind) {
  const [x, y] = [
    oklab(kind ? simulate(a, kind) : a),
    oklab(kind ? simulate(b, kind) : b),
  ];
...
const TEXT_PAIRS = [
  ...
  [
    ...
    "--color-bg-sidebar",
    4.5,
    "muted text on the activity bar",
  ],
  [
    "--color-fg-icon",
    "--color-bg-sidebar",
  ...
  [
    ...
    "--color-bg-base",
    4.5,
    "a { color: accent } on the window",
  ],
  [
    "--color-accent",
    "--color-bg-surface",
    4.5,
    "a { color: accent } on a panel",
  ],
  [
    "--color-accent",
    "--color-accent-subtle",
...
function checkTheme(name, vars, report) {
  ...
  for (const [fgT, bgT, min, what] of TEXT_PAIRS) {
    ...
    const bg = get(bgT);
    if (!fg || !bg) {
      fail(`missing token in pair ${fgT} / ${bgT}`);
      continue;
    }
  ...
  for (const g of GRADES) {
    ...
    if (!sw || !surface || !onReview) {
      fail(`missing --color-review-${g} / --color-on-review`);
      continue;
    }
    ...
    if (label < 4.5)
      fail(
        `${label.toFixed(2)} < 4.5 — "${g}" label on its fill (.trainer-grade)`,
      );
    if (asText < 4.5)
      fail(
        `${asText.toFixed(2)} < 4.5 — "${g}" as text on a panel (.sum--${g} b)`,
      );
    if (onTint < 4.5)
      fail(`${onTint.toFixed(2)} < 4.5 — "${g}" verdict text on its 8% tint`);
  }
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (!hl || !fg) {
      fail(`missing --color-hl-${i}`);
      continue;
    }
    const c = contrast(fg, hl);
    if (c < 4.5)
      fail(`${c.toFixed(2)} < 4.5 — document text on highlight ${i}`);
  }
  ...
  for (const [n, c] of nodes) {
    if (!c) {
      fail(`missing --color-graph-${n}`);
      continue;
    }
    const ct = contrast(c, canvas);
    if (ct < 3)
      fail(`${ct.toFixed(2)} < 3 — graph "${n}" node against the canvas`);
  }
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (normal < 15)
        fail(
          `${normal.toFixed(1)} < 15 — graph nodes "${na}"/"${nb}" too alike (normal vision)`,
        );
      if (cvd < 8)
        fail(
          `${cvd.toFixed(1)} < 8 — graph nodes "${na}"/"${nb}" too alike under colour-blindness`,
        );
    }
  }
  ...
  for (const t of ["link", "disconnect", "inherit"]) {
    ...
    if (!c) {
      fail(`missing --color-graph-${t}`);
      continue;
    }
    const ct = contrast(c, canvas);
    if (ct < 3)
      fail(`${ct.toFixed(2)} < 3 — graph "${t}" link against the canvas`);
  }
  ...
  if (!edge) fail("missing --color-graph-edge");
    ...
    if (ct < 1.8)
      fail(
        `${ct.toFixed(2)} < 1.8 — graph resting links invisible on the canvas`,
      );
    const loudest = Math.min(
      ...nodes.filter(([, c]) => c).map(([, c]) => contrast(c, canvas)),
    );
    if (ct > loudest) {
      fail(
        `resting links (${ct.toFixed(2)}) out-contrast the quietest node (${loudest.toFixed(2)}) — structure should recede`,
      );
    }
  }
  ...
  if (hover && base && surface) {
    ...
    if (vsSurface < 1.12 || vsBase < 1.12) {
      fail(
        `hover barely differs from its backdrop (panel ${vsSurface.toFixed(3)}, window ${vsBase.toFixed(3)}, want >= 1.12)`,
      );
    }
  }
}
...
if (names.length === 0) {
  console.error(
    "No [data-theme] blocks found in src/ui/index.css — did the file move?",
  );
  process.exit(1);
}
...
if (failures.length) {
  console.error(
    `Theme contrast check failed (${failures.length} issue${failures.length === 1 ? "" : "s"}):\n`,
  );
...
console.log(
  `Theme contrast check passed — ${names.length} themes: ${names.join(", ")}`,
);
