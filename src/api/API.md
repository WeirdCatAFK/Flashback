...
### How the policy table is applied

`api.js` mounts the guard alongside each router — `app.use('/api/documents', guard('documents'), documentsRouter)`. Express strips the mount prefix inside a `use` middleware, so the path the guard sees is the path *within* the router (`/list`, `/metadata`), which is the granularity the rules need without touching a single handler.

A rule reads `[method, pathPattern, minimumRole]`. `method` is an HTTP verb or `*`; `pathPattern` is `*`, a literal path, or a prefix ending in `/*`. First match wins, so rules run most specific to least and every mount ends in a catch-all. A mount with no entry resolves to `author` rather than to "allowed": a new router nobody registered stops working immediately and loudly, which is the correct direction for that mistake to fall.

The alternative — a role check inside each handler — was rejected because its failure mode is silence: a handler that forgets its check is indistinguishable from one meant to be open, and nothing in the file says which.

### Why `accounts` is the one inverted mount
...
| `name`      | string | Yes      | The asset'soriginal Anki filename, e.g. `taberu.mp3`. |
...
| `excludeTag`      | string | Hold back cards carrying thiseffective tag. Repeatable.          |
...
| Param        | Type   | Description                                                                                                                                |
| ------------ | ------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `q`        | string | Free-text query. Alone, this selectsglobal mode.                                                                                 |
...
| `cursor` | query | string | No       | Oid of the last commit already held; the page resumesafter it. Omit for the newest page. |
...
| `schemaVersion`    | number       | Highest applied migration — describes thisdatabase.                                                                                                                              |
