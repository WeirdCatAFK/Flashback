...
- `getMaxAccounts()` — the most active accounts the install may hold (`maxAccounts`, from `FLASHBACK_MAX_ACCOUNTS`), or `null` for no cap. Read fresh from disk like `getMcpDiaryAccess()`, so an operator editing `config.json` on the volume needs no restart. Enforced by `routes/accounts.js`, never by `accounts.createAccount()` — that is also what provisions the Author at boot.
...
Progress is scoped to a person. Every method that reads or writes a schedule, a review log, a
health verdict or a fitted weight vector takes a `scope`. `scoped()` refuses a missing one
rather than defaulting, because defaulting to the owner would hand the owner's schedule to
whoever forgot the argument, silently. `PROGRESS_JOIN` left-joins `CardProgress`; a missing row
means "never reviewed by this person", which is exactly what a zero level and a NULL
`last_recall` meant when those columns lived on `Flashcards` — so every reader COALESCEs and
nothing is seeded on card creation.

Bind ordering in `getDueFlashcards`. The statement is assembled from optional pieces, so binds
are grouped by *where they appear in the finished SQL* and concatenated in that sequence:
`folderParams` (the `folder_tree` CTE), `excludeFolderParams` (`excluded_tree`), `efParams`
(`latest_ef`, SM-2 only), then `cardsParams` (the `cards` CTE — its progress join first, then its
WHERE filters). A single flat array worked while only the WHERE clause had binds; it stopped
working the moment the account scope had to appear inside two of the CTEs. Within `cardsParams`
the order is the order `whereConditions` was pushed in. A filter whose bind lives in a CTE pushes
a condition and no param, so the two lists are not the same length and must not be zipped.

Batched reads exist where the per-row form is a session-sized N+1. `getSessionFacets(hashes)`
returns `Map<globalHash, { docId, folderId, ancestorIds, tags, deckIds, linkedDocIds }>` for a
whole session in a fixed number of statements. Per-card `getInheritedTagNames` +
`getDirectTagNames` would be two statements per card — ~400 for a 200-card session — on a path
that runs on every `/due`. A standalone card has `docId`/`folderId` null and still carries its
tags and decks, so it is never confusable by location alone.

Two card-health methods deliberately take no scope. `deleteAllCardFlags` and
`resetAllCardHealth` are the edit hook: when a card's content changes, every account's verdict is
about a card that no longer exists, so every account's flags go and every watermark moves. The
fingerprint check in `buildContext` would reach the same state one failure at a time; these make
it happen the moment the user saves, rather than leaving a reader looking at a verdict on text
they can see has been rewritten.

`presence` is the owner's; the graph rollups are the viewer's. `getFlashcardAvgLevel` feeds
`Documents.presence`, a stored, sidecar-mirrored field that travels with the document — so its
caller always passes `OWNER_SCOPE`, and a reader's grade never rewrites what the sidecar claims.
The `CARD_LEARNED_SQL` rollups in `getGraphData` are the opposite: how well *this viewer* knows
the material, read against their own progress.
...
- Sidecar and body writes are **atomic**: `_atomicWrite()` writes a `<file>.tmp-<pid>-<ms>` sibling and renames it into place, removing the sibling if the write fails. A full disk or quota therefore leaves the previous file intact rather than a truncated one, and the raw `ENOSPC`/`EDQUOT` error travels up unchanged so `src/api/httpErrors.js` can answer `507 storage_full`. New empty bodies and media blobs keep the plain write: a truncated brand-new file is not data loss, and media is addressed by its hash. A sibling that outlives a crash is reported by the Doctor as an untracked file, which is the right verdict for it.
...
Internals. Both acquire loops are wake-all-and-re-check rather than hand-off: whoever is
woken re-tests its own condition and re-queues if it lost the race. That costs a few microtasks
under contention and removes the class of bug where a hand-off resolves a waiter whose condition
changed between the wake and the resume. A document write queues behind a structural operation
that *holds* the tree or is waiting for it — waiting counts, and that is the whole
anti-starvation rule; without it a steady stream of edits postpones a move indefinitely. On
release, writers are woken first, so a second structural operation does not queue behind the
documents that piled up behind the first. The per-path chain tail is held in a variable rather
than recomputed: `previous.then(...)` returns a new promise on every call, so comparing against a
fresh one would never match and the map would grow by one entry per document ever written.
...
Scope filters are applied in SQL, never after the fact. `getDue()` slices `maxNew` inside the
statement, so a filter applied to the rows it returned would quietly turn "20 new cards" into
however few of them survived the filter.
